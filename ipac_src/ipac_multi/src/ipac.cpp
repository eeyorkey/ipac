...
int ipac::Eval(real_t& loglik) {
	...
	for (size_t i = 0; i < batch.size; i ++) {
		...
		real_t prod_k_zero = 0.0;
		for (int k = 0; k < _d_J; k ++) {
			real_t beta_var = beta(_alpha[i], _data_labels[i][k]);
			prod_k_one += log(_Q[k][1] * beta_var + 1 - _Q[k][1]);
			prod_k_zero += log(_Q[k][0] * beta_var + 1 - _Q[k][0]);
		}
...
{
    FILE* f_writer = fopen(info_file, "w");
    if (f_writer == NULL) {
        return;
    }
    ...
    fprintf(f_writer, "Q");
    for (int i = 0; i < _d_J; i ++) {
        fprintf(f_writer, " %f,%f", _Q[i][0], _Q[i][1]);
	}
    fprintf(f_writer, "\n");
    
    // dump alpha
    fprintf(f_writer, "alpha");
    for (int i = 0; i < _d_J; i ++) {
        fprintf(f_writer, " %f", _alpha[i]);
    }
    fprintf(f_writer, "\n");
    ...
    // stage one pi
    fprintf(f_writer, "stageone_pi1");
    fprintf(f_writer, " %f\n", _stage_one_pi1);
    
    // loglik stage one
    fprintf(f_writer, "loglik_stag_1");
    for (int i = 1; i < _stage_one_loglik.size(); i ++) {
        fprintf(f_writer, " %f", _stage_one_loglik[i]);
    }
    fprintf(f_writer, "\n");
    
    // loglik stage two
    fprintf(f_writer, "loglik_stag_2");
    for (int i = 1; i < _stage_two_loglik.size(); i ++) {
        fprintf(f_writer, " %f", _stage_two_loglik[i]);
    }
    fprintf(f_writer, "\n");
