...
int ipac::LoadData(const char* f_name, std::string tag) {
	...
	if (tag == "train") {
		...
		_d_D = _data_mat_train->info.info.num_col;  // num of features

		// check data, p_value ranges: p_value >= 0 && p_value <= 1
		std::vector<float>& tmp_labels = _data_mat_train->info.labels;

		for (int i = 0; i < tmp_labels.size(); i ++) {
			float p = tmp_labels[i];
		...
		std::vector<float>& tmp_labels = _data_mat_test->info.labels;
...
void ipac::init_pi1(std::vector<real_t>& vec) {
	...
	for (size_t i = 0; i < batch.size; i ++) {
		real_t sum_out = 0.0;
		for (size_t k = 0; k < batch[i].length; k ++) {
			size_t index = batch[i][k].index;
			real_t value = (real_t) batch[i][k].fvalue;
			sum_out += value * _model_weight[index];
		}
		sum_out += _model_bias;
		
		vec[i] = sigmod(sum_out);
	}
}
...
#pragma omp parallel for reduction(+:sum_z1,sum_z1p) if( coreNum > 1) num_threads(coreNum)
			for (int t = cv * each_fold_num; 
				...
				sum_z1 += Z1[j];
				sum_z1p += Z1[j] * 
					(-log(_data_mat_train->info.labels[j]));
			}
			// end expectation
...
#pragma omp parallel for reduction(+:sum_error) if( coreNum > 1) num_threads(coreNum)
			for (int t = cv * each_fold_num; 
				...
				for (int k = 0; k < batch[j].length; k ++) {
					size_t index = batch[j][k].index;
					real_t value = (real_t) batch[j][k].fvalue;
					sum_out += value * _model_weight[index];
				}
				sum_out += _model_bias;

				real_t pred = sigmod(sum_out);
				real_t error = (Z1[j] - pred);
				sum_error += error;
				...
				for (int k = 0; k < batch[j].length; k ++) {
					...
					size_t index = batch[j][k].index;  // update grad for non-zero features
					real_t value = (real_t) batch[j][k].fvalue;
...
#pragma omp parallel for reduction(+:sum_loglik) if( coreNum > 1) num_threads(coreNum)
			for (int t = cv * each_fold_num; 
				...
				size_t j = t % _d_M;
				real_t sum_out = 0.0;
				for (size_t k = 0; k < batch[j].length; k ++) {
					size_t index = batch[j][k].index;
					real_t value = (real_t) batch[j][k].fvalue;
					sum_out += value * _model_weight[index];
				}
				sum_out += _model_bias;
				real_t pred_var = sigmod(sum_out);
				pred_1[j] = pred_var;

				beta_var[j] = beta(_alpha, 
					_data_mat_train->info.labels[j]);

				// p(X|theta)=P(X|Z=0;theta)p(Z=0)+P(X|Z=1;theta)p(Z=1)
				sum_loglik += log((1-pred_1[j]) + pred_1[j] * beta_var[j]);
			}
...
#pragma omp parallel for reduction(+:sum_loglik_valid) if( coreNum > 1) num_threads(coreNum)
				for (int t = cv * each_fold_num + train_fold_num * each_fold_num; 
					...
					sum_loglik_valid += log( (1-pred_var) + 
						pred_var * beta(_alpha, _data_mat_train->info.labels[j]) );
				}
...
int ipac::Eval(real_t& loglik) {
	...
	for (size_t i = 0; i < batch.size; i ++) {
		...
		sum_out += _model_bias;
		real_t pred_var = sigmod(sum_out);
		pred_var = pred_var > 0.999? 0.999: pred_var;
		pred_var = pred_var < 0.001? 0.001: pred_var;

		loglik += log( (1-pred_var) + 
			pred_var * beta(_alpha, _data_mat_test->info.labels[i]) );
	}
...
int ipac::dump_model(const char* f_name)
{
	...
	f_writer.write((const char*)&weight_dim, sizeof(size_t));
